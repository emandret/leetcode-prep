...
class SegmentTreeMinimumRangeQuery
{
  ...
  private:
    ...
    void constructMinSegmentTree(const vector<int>& input, int low, int high, int pos);

    int rangeMinimumQuery(const vector<int>& input, int low, int high, int qlow, int qhigh, int pos);
